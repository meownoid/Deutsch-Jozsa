...
preprocess str
  ...
  | not (all validWord args) = Left "Incorrect argument name"
  | null right = Left "Expecting '='"
  | otherwise = Right (name, args, right)
  where
    (left : right : _) = splitBy '=' str ++ [[]]
...
parseFunction str = case preprocess str of
  ...
  Right (name, args, expr) -> case parse expressionParser "function" expr of
    Left e -> Left $ show e
    Right pexpr -> (name,length args,) <$> postprocess args pexpr
